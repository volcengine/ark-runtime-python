...
async def main():
    ...
    async for event in stream:
        ...
        if isinstance(event, ResponseCompletedEvent):
            response_id = event.response.id
        if isinstance(event, ResponseOutputItemDoneEvent) and isinstance(event.item, ItemFunctionToolCall):
    ...
    async for event in stream:
        ...
        if isinstance(event, ResponseCompletedEvent):
            response_id = event.response.id
        if isinstance(event, ResponseOutputItemDoneEvent) and isinstance(event.item, ItemFunctionMcpApprovalRequest):
