...
from httpx import URL, AsyncClient, Client, Timeout
...
from ._utils._key_agreement import get_cert_info, key_agreement_client
...
class Ark(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        # Reject empty api_key eagerly — passing "" would otherwise skip the
        # `is not None` guard, produce an "Authorization: Bearer " header
        # with no token, and surface as an opaque ArkAPIConnectionError
        # ("Connection error") from the HTTP layer instead of a clear
        # config-time failure.
        if api_key is not None and not api_key.strip():
            raise ValueError(
                "api_key must be a non-empty string; got '' — supply a valid API "
                "key, set ARK_API_KEY, or pass ak/sk instead"
            )
        ...
        # Managed Agents (data plane /api/v3/*)
        self.agents = resources.Agents(self)
        self.environments = resources.Environments(self)
        self.memory_stores = resources.MemoryStores(self)
        self.sessions = resources.Sessions(self)
        self.skills = resources.Skills(self)
        self.vaults = resources.Vaults(self)

    @classmethod
    def volc(cls, **kwargs) -> "Ark":
        """Construct an Ark client targeting the Volcengine cloud
        (``ark.cn-beijing.volces.com``). Reads ``VOLC_ACCESSKEY`` /
        ``VOLC_SECRETKEY`` for AK/SK and ``ARK_API_KEY`` for the API key
        when those kwargs are not passed explicitly."""
        return _from_cloud(cls, Cloud.VOLC, **kwargs)

    @classmethod
    def byteplus(cls, **kwargs) -> "Ark":
        """Construct an Ark client targeting the Byteplus cloud
        (``ark.ap-southeast.bytepluses.com``). Reads
        ``BYTEPLUS_ACCESSKEY`` / ``BYTEPLUS_SECRETKEY`` and ``ARK_API_KEY``
        when those kwargs are not passed explicitly."""
        return _from_cloud(cls, Cloud.BYTEPLUS, **kwargs)
...
class AsyncArk(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        self.environments = resources.AsyncEnvironments(self)
        self.memory_stores = resources.AsyncMemoryStores(self)
        self.sessions = resources.AsyncSessions(self)
        self.skills = resources.AsyncSkills(self)
        self.vaults = resources.AsyncVaults(self)

    @classmethod
    def volc(cls, **kwargs) -> "AsyncArk":
        """See :meth:`Ark.volc`."""
        return _from_cloud(cls, Cloud.VOLC, **kwargs)

    @classmethod
    def byteplus(cls, **kwargs) -> "AsyncArk":
        """See :meth:`Ark.byteplus`."""
        return _from_cloud(cls, Cloud.BYTEPLUS, **kwargs)
...
def _from_cloud(cls, cloud: str, **kwargs):
    """Resolve a cloud preset, fill in env-var defaults for the cloud's
    specific AK/SK names plus the shared ``ARK_API_KEY``, then delegate to
    ``cls.__init__``. Explicit kwargs always win."""
    preset = _CLOUD_PRESETS[cloud]
    kwargs.setdefault("base_url", preset["base_url"])
    kwargs.setdefault("region", preset["region"])
    if "ak" not in kwargs:
        kwargs["ak"] = os.environ.get(preset["ak_env"])
    if "sk" not in kwargs:
        kwargs["sk"] = os.environ.get(preset["sk_env"])
...
class StsTokenManager(object):
    ...
    def _protected_refresh(
        ...
        try:
            api_key, expired_time = self._load_api_key(ep, ttl, resource_type=resource_type, project_name=project_name)
    ...
    def _refresh(self, ep: str, resource_type: str = _DEFAULT_RESOURCE_TYPE, project_name: str = None):
        ...
        if self._refresh_lock.acquire(False):
            ...
            try:
                is_mandatory_refresh = self._need_refresh(ep, self._mandatory_refresh_timeout)
            ...
            with self._refresh_lock:
                ...
                self._protected_refresh(ep, is_mandatory=True, resource_type=resource_type, project_name=project_name)
    ...
    def _load_api_key(
        ...
        resp: volcenginesdkark.GetApiKeyResponse = self.api_instance.get_api_key(get_api_key_request)
...
class E2ECertificateManager(object):
    ...
    def _load_cert_by_ak_sk(self, ep: str) -> str:
        get_endpoint_certificate_request = volcenginesdkark.GetEndpointCertificateRequest(id=ep)
        if self._aicc_enabled:
            get_endpoint_certificate_request = volcenginesdkark.GetEndpointCertificateRequest(id=ep, type="AICCv0.1")
        try:
    ...
    def _init_local_cert_cache(self):
        ...
        if not os.path.exists(self._cert_storage_path):
            try:
                ...
                raise ArkAPIError("failed to create certificate directory %s: %s\n" % (self._cert_storage_path, e))
