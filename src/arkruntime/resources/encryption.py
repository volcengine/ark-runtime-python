...
def _decrypt_chunk(key: bytes, nonce: bytes, resp: Stream[ChatCompletionChunk]) -> Iterator[ChatCompletionChunk]:
    for chunk in resp:
        ...
        if resp.choices is not None:
            for index, choice in enumerate(resp.choices):
                if (
                    ...
                    try:
                        content = aes_gcm_decrypt_base64_string(key, nonce, choice.message.content)
                    except Exception:
                        content = ""
                    if content == "" or not decrypt_validate(choice.message.content):
                        content = aes_gcm_decrypt_base64_list(key, nonce, choice.message.content)
...
async def _async_decrypt(
    ...
    if isinstance(resp, ChatCompletion):
        ...
        return AsyncStream._make_stream_from_iterator(_async_decrypt_chunk(key, nonce, resp))
                ...
                for part in current_content:
                    if part.get("type", None) == "text":
                        ...
                        if parse_result.scheme == "data":
                            ...
                            raise TypeError("encryption is not supported for image url scheme {}".format(parse_result))
                    elif part.get("type", None) == "video_url":
                        pass
                    else:
                        raise TypeError("encryption is not supported for content type {}".format(type(part)))
            else:
                raise TypeError("encryption is not supported for content type {}".format(type(message.get("content"))))
