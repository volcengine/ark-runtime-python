...
import httpx
import pydantic
from typing_extensions import ParamSpec, get_origin, override
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__("The `stream` argument was set to `True` but the `stream_cls` argument was not given.")
...
def to_streamed_response_wrapper(
    ...
    @functools.wraps(func)
    def wrapped(*args: P.args, **kwargs: P.kwargs) -> ResponseContextManager[ArkAPIResponse[R]]:
        extra_headers: dict[str, str] = {**(cast(Any, kwargs.get("extra_headers")) or {})}
        ...
        return ResponseContextManager(cast(Callable[[], ArkAPIResponse[R]], make_request))
...
def async_to_streamed_response_wrapper(
    ...
    @functools.wraps(func)
    def wrapped(*args: P.args, **kwargs: P.kwargs) -> AsyncResponseContextManager[ArkAsyncAPIResponse[R]]:
        extra_headers: dict[str, str] = {**(cast(Any, kwargs.get("extra_headers")) or {})}
        extra_headers[RAW_RESPONSE_HEADER] = "stream"
        ...
        return AsyncResponseContextManager(cast(Awaitable[ArkAsyncAPIResponse[R]], make_request))
