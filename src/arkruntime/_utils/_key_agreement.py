...
def get_cert_info(cert_pem: str) -> Tuple[str, str, float]:
    ...
    try:
        dns = cert.extensions.get_extension_for_class(x509.SubjectAlternativeName).value.get_values_for_type(
            x509.DNSName
        )
...
class key_agreement_client:
    def __init__(self, certificate_pem_string: str) -> None:
        ...
        self._public_key = ec.EllipticCurvePublicNumbers(cert_pub.x, cert_pub.y, self._curve).public_key()
    ...
    def init_cert_ring_key_id(self) -> None:
        ...
        try:
            ...
            dns = self._cert.extensions.get_extension_for_class(x509.SubjectAlternativeName).value.get_values_for_type(
                x509.DNSName
            )
            if dns and len(dns) > 1 and re.match(r"^ring\..*$", dns[0]) and re.match(r"^key\..*$", dns[1]):
