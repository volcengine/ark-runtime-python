...
from ._models import GenericModel, construct_type
...
from ._utils._utils import _gen_request_id, is_dict, is_given, is_list, is_mapping
...
class BaseClient(Generic[_HttpxClientT]):
    ...
    def _make_status_error(
        ...
        if response.status_code == 400:
            return _exceptions.ArkBadRequestError(err_msg, response=response, body=data, request_id=request_id)

        if response.status_code == 401:
            return _exceptions.ArkAuthenticationError(err_msg, response=response, body=data, request_id=request_id)

        if response.status_code == 403:
            return _exceptions.ArkPermissionDeniedError(err_msg, response=response, body=data, request_id=request_id)

        if response.status_code == 404:
            return _exceptions.ArkNotFoundError(err_msg, response=response, body=data, request_id=request_id)

        if response.status_code == 409:
            return _exceptions.ArkConflictError(err_msg, response=response, body=data, request_id=request_id)

        if response.status_code == 422:
            return _exceptions.ArkUnprocessableEntityError(err_msg, response=response, body=data, request_id=request_id)

        if response.status_code == 429:
            return _exceptions.ArkRateLimitError(err_msg, response=response, body=data, request_id=request_id)

        if response.status_code >= 500:
            return _exceptions.ArkInternalServerError(err_msg, response=response, body=data, request_id=request_id)
        return ArkAPIStatusError(err_msg, response=response, body=data, request_id=request_id)

    def _make_status_error_from_response(self, response: httpx.Response, request_id: str) -> ArkAPIStatusError:
        ...
        return self._make_status_error(err_msg, body=body, response=response, request_id=request_id)
    ...
    def _process_response_data(
        ...
        *,
        ...
        try:
            ...
            raise ArkAPIResponseValidationError(response=response, body=data, request_id=request_id) from err
...
class SyncAPIClient(BaseClient):
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise self._make_status_error_from_response(err.response, request_id=req_id) from None
    ...
    def get(
        ...
        *,
        ...
        opts = RequestOptions.construct(
            method="get",
            url=path,
            # query params flow in via `options["params"]` (set by
            # make_request_options); no separate `params` kwarg.
            **options,
        )
    ...
    def post_without_retry(
        ...
        *,
        ...
        return cast(
            ResponseT,
            self.request(cast_to, opts, remaining_retries=0, stream=stream, stream_cls=stream_cls),
        )
