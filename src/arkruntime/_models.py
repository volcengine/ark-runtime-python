...
def construct_type(*, value: object, type_: object) -> object:
    ...
    # unwrap `TypeAliasType` (PEP 695-style aliases emitted by codegen for
    # union root models) -> the aliased type
    if type(type_) is TypeAliasType:
    ...
    # Coerce a raw wire scalar into the declared Enum. Codegen emits fields
    # like ``Session.status: SessionStatus`` (direct Enum type — no
    # ``Optional[...]`` / ``Union[...]`` wrapper), and the union branch above
    # never fires for those. Without this fallback, ``.status`` comes back as
    # the raw ``str`` from the wire, so ``session.status == SessionStatus.idle``
    # is silently False. ``inspect.isclass`` gate avoids ``issubclass`` on
    # non-class objects (Annotated, Literal, TypeAliasType — already unwrapped
...
def _build_discriminated_union_meta(*, union: type, meta_annotations: tuple[Any, ...]) -> DiscriminatorDetails | None:
    ...
    for annotation in meta_annotations:
        if isinstance(annotation, PropertyInfo) and annotation.discriminator is not None:
            ...
            break
        # codegen emits pydantic-native `Field(discriminator=...)` annotations
        if isinstance(annotation, FieldInfo) and annotation.discriminator is not None:
            discriminator_field_name = annotation.discriminator
            break
